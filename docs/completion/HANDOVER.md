...
1. **Chosen Poké Ball doesn't vanish from the lab-back table** — still OPEN. Cosmetic. The lab-back map (`maps/SilentHillLabBack.asm`) defines 3 `SPRITE_POKE_BALL` objects and has **unused single-ball NPCID lists** (`SilentHillLabBackNPCIDs2/3/4`) that were clearly meant to be swapped in after choosing but were never wired. Fix options: switch the post-choice scene to a reduced NPCID set, or hide the taken ball(s) via `Movement_hide_object`/`remove_object`. Needs playtest to confirm intended behavior (which balls disappear and when). Investigate the scene→NPCID mechanism in `data/maps/scenes.asm` + `home/menu.asm` `GetMenuIndexSet` pattern.
